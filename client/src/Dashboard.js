...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5001/allprofiles", {
        headers: {
          "x-token": localStorage.getItem("token"),
        },
      })
        ...
        {!loading && data.length > 0 ? (
          data.map((profile) => (
            ...
            >
              ...
              <Link to={`/indprofile/${profile.fullname}/${profile.email}/${profile.skill}/${profile._id}`} className="btn"
                style={{backgroundColor: "#007bff",color: "white",padding: "8px 12px",textDecoration: "none",
                  borderRadius: "4px",display: "inline-block",marginTop: "10px",
                }}
              >
                View Profile
              </Link>
              <ul>
                {profile.skill.split(",").map(skill =>
                  <li className = "text-primary">
                    <i className="fas fa-check"><i>{skill}</i></i>
                  </li>
                )}
              </ul>
            </div>
